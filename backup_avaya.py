...
for ip_address in hosts:
	...
	if is_valid_ip(ip_address):
		try :
			...
			telnet.read_until("***************************************************************",timeout) # Session indicator for Avaya's switches
			telnet.read_until("***************************************************************",timeout) # Session indicator for Avaya's switches (It takes two to indicate when we can enter the accounts)
			...
			telnet.write("copy running-config tftp address " + tftp_server + " filename " + host_name + "\n") # Switch command for saving configuration in tftp
